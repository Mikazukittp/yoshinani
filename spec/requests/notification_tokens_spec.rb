RSpec.describe 'NotificationToken', type: :request do
  ...
  describe 'POST /api/oauth_registration' do
    ...
    context '初回作成の場合' do
      before do
        post api_notification_tokens_path, params, env
    ...
    context 'すでに同じtokenが存在している場合' do
      before do
        create(:notification_token, device_token: 'hogehoge', user_id: sign_in_user.id)
        post api_notification_tokens_path, params, env
        @json = JSON.parse(response.body)
      end
  ...
  describe 'PATCH /api/oauth_registration' do
    let!(:old_token) { create(:notification_token, device_token: 'fugafuga', user_id: sign_in_user.id) }
    let(:params) {
      {
        notification_token: {
    ...
    before do
      patch api_notification_tokens_path, params, env
      @json = JSON.parse(response.body)
    end
  ...
  describe 'DELETE /api/oauth_registration' do
    ...
    before do
      delete api_notification_tokens_path, params, env
      @json = JSON.parse(response.body)
    end
